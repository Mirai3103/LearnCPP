...
void ChucNang10(LinkedList &list)
{
    ...
    for (Node *Inode = list.getFirstNode(); Inode != NULL; Inode = Inode->next)
    {
        fprintf(fp, "Ma mat hang: %d, ten mat hang: %s, don vi: %s, can nang: %.3f, gia: %.3f\n", Inode->value.ItemCode, Inode->value.ItemName, Inode->value.ItemUnit, Inode->value.ItemWeight, Inode->value.ItemPrice);
    }
    fclose(fp);
}
int main()
